...
init:
    ...
    BSF TRISA, 0 ; pinMode ((PORTA) and 07Fh), 0 input(1)
    BSF TRISA, 1 ; pinMode ((PORTA) and 07Fh), 1 input(1)
    BCF TRISA, 3 ; pinMode ((PORTA) and 07Fh), 3 output(0)

    BCF STATUS, 6 ; set RP6 to 0
    BCF STATUS, 5 ; set RP5 to 0 Now in bank 0

    MOVLW 0x07 ; set W to 0x07
    MOVWF CMCON ; set CMCON to W
    BCF PORTA, 3 ; set ((PORTA) and 07Fh), 2 to 0
    RETURN
work:
loop:
    BTFSC PORTA, 1 ; is ((PORTA) and 07Fh), 1 == 1 then
        GOTO one
    BTFSC PORTA, 0 ; is ((PORTA) and 07Fh), 0 == 1 then
        BSF PORTA, 3 ; set ((PORTA) and 07Fh), 3 to 1
    GOTO loop
one:
    BCF PORTA, 3 ; set ((PORTA) and 07Fh), 2 to 0
    GOTO loop
