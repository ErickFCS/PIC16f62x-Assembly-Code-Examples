...
main:
 ...
 ORG 0x00
    ...
    BSF TRISB, 0 ; pinMode ((PORTB) and 07Fh), 0 input(1)
    BSF TRISB, 1 ; pinMode ((PORTB) and 07Fh), 1 input(1)
    BCF TRISB, 3 ; pinMode ((PORTB) and 07Fh), 3 output(0)

    BCF STATUS, 6 ; set RP6 to 0
    BCF STATUS, 5 ; set RP5 to 0 Now in bank 0

    BCF PORTB, 3 ; set ((PORTB) and 07Fh), 3 to 0
    ;RETURN

loop:
    BTFSC PORTB, 1 ; is ((PORTB) and 07Fh), 1 == 1 then
        BCF PORTB, 3 ; set ((PORTB) and 07Fh), 3 to 0
    BTFSS PORTB, 1 ; is ((PORTB) and 07Fh), 1 == 0 then
        call else1
    GOTO loop
    ;RETURN

else1:
    BTFSC PORTB, 0 ; is ((PORTB) and 07Fh), 0 == 1 then
        BSF PORTB, 3 ; set ((PORTB) and 07Fh), 3 to 1
    RETURN
