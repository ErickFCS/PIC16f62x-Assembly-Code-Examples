...
init:
    ...
    BSF TRISA, 4 ; pinMode ((PORTA) and 07Fh), 4 input(1)
    BSF TRISA, 5 ; pinMode ((PORTA) and 07Fh), 5 input(1)
    BCF TRISB, 1 ; pinMode ((PORTB) and 07Fh), 1 output(0)
    BCF TRISB, 2 ; pinMode ((PORTB) and 07Fh), 2 output(0)
    BCF TRISB, 7 ; pinMode ((PORTB) and 07Fh), 7 output(0)

    BCF STATUS, 6 ; set RP6 to 0
    BCF STATUS, 5 ; set RP5 to 0 Now in bank 0

    BCF PORTB, 1 ; set ((PORTB) and 07Fh), 1 to 0
    BCF PORTB, 2 ; set ((PORTB) and 07Fh), 2 to 0
    BCF PORTB, 7 ; set ((PORTB) and 07Fh), 7 to 0
    RETURN
work:
loop:
    BTFSC PORTA, 4 ; is ((PORTA) and 07Fh), 4 == 1 then
        call if1
    BTFSS PORTA, 4 ; is ((PORTA) and 07Fh), 4 == 0 then
        call else1
    GOTO loop
    RETURN
...
else1:
    BTFSC PORTA, 5 ; is ((PORTA) and 07Fh), 5 == 1 then
        call if2
    RETURN

if2:
    BSF PORTB, 1 ; set ((PORTB) and 07Fh), 1 to 1
    BSF PORTB, 2 ; set ((PORTB) and 07Fh), 2 to 1
    BSF PORTB, 7 ; set ((PORTB) and 07Fh), 7 to 1
    RETURN
